...
function getWeather()
{
    ...
    $weather = S('weather');
    if (!$weather) {
        $apiUrl = "http://op.juhe.cn/onebox/weather/query?cityname=宜宾&key=您申请的KEY";
        $backData = \Common\Helper\HttpHelper::http($apiUrl);
        $obj = json_decode($backData, true);
        if (intval($obj['error_code']) == 0) {
            $weather = $obj['result']['data']['realtime']['weather'];
        }
...
function getSlider()
{
    ...
    return M('silder')->where($map)->order('priorityr DESC')->select();
}
